...
void pinMode_generateCallFrame(arpcDataFrame_t *frame, uint8_t pin, uint8_t mode){
...
void digitalWrite_generateCallFrame(arpcDataFrame_t *frame, uint8_t pin, uint8_t val){
...
void analogWrite_generateCallFrame(arpcDataFrame_t *frame, uint8_t pin, int32_t val){
...
void delayMicroseconds_generateCallFrame(arpcDataFrame_t *frame, uint32_t us){
   ...
   uint8_t parameters[sizeof(us)] = { 0 };
    ...
    arpcEncodeGeneric(frame, delayMicroseconds_ID, parameters, sizeof(parameters));
...
void pulseIn_generateCallFrame(arpcDataFrame_t *frame, uint8_t pin, uint8_t state, uint32_t timeout){
...
void pulseInLong_generateCallFrame(arpcDataFrame_t *frame, uint8_t pin, uint8_t state, uint32_t timeout){
...
void shiftOut_generateCallFrame(arpcDataFrame_t *frame, uint8_t dataPin, uint8_t clockPin, uint8_t bitOrder, uint8_t val){

   uint8_t parameters[sizeof(dataPin) + sizeof(clockPin) + sizeof(bitOrder) + sizeof(val)] = { 0 };

    memcpy(parameters, &dataPin, sizeof(dataPin));
    memcpy(parameters + sizeof(dataPin), &clockPin, sizeof(clockPin));
    memcpy(parameters + sizeof(dataPin) + sizeof(clockPin), &bitOrder, sizeof(bitOrder));
    memcpy(parameters + sizeof(dataPin) + sizeof(clockPin) + sizeof(bitOrder), &val, sizeof(val));
...
void shiftIn_generateCallFrame(arpcDataFrame_t *frame, uint8_t dataPin, uint8_t clockPin, uint8_t bitOrder){

   uint8_t parameters[sizeof(dataPin) + sizeof(clockPin) + sizeof(bitOrder)] = { 0 };
